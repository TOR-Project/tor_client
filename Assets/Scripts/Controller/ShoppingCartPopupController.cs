...
public class ShoppingCartPopupController : MonoBehaviour
{
    [SerializeField]
    Text titleText;
    [SerializeField]
    ...
    public void showPopup(int _itemId, bool _isBuyMode)
    {
        ...
        warningText.text = LanguageManager.instance.getText(_isBuyMode ? "ID_CANT_BUYABLE_ITEM" : "ID_CANT_SELLABLE_ITEM");
        ...
        bool buyable = _isBuyMode && sellItemData.remainCount != 0 && (sellItemData.deadline <= 0 || SystemInfoManager.instance.blockNumber < sellItemData.deadline);
        bool sellable = !_isBuyMode && sellItemData != null && InventoryManager.instance.getMyItemCount(_itemId) > 0;

        amountPanel.SetActive(buyable || sellable);
        pricePanel.SetActive(buyable || sellable);
        warningPanel.SetActive((_isBuyMode && !buyable) || (!_isBuyMode && !sellable));
        confirmButton.interactable = buyable || sellable;
